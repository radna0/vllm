#!/usr/bin/env python3
"""
Comprehensive benchmark script for VLLM speculative decoding validation using OpenAI API server.

Tests multiple configurations and collects detailed metrics from VLLM server endpoints.
"""
...
class VLLMServerBenchmark:
    """Run benchmarks using VLLM server with OpenAI API client."""
    ...
    def create_server_command(
        ...
        **kwargs
        ...
        if method == "baseline":
            # No speculation - just base model
            return cmd_base
    ...
    def start_server(self, method: str = "baseline", num_spec_tokens: int = 3, **kwargs) -> bool:
        """Start VLLM server with specific configuration."""
        ...
        try:
            cmd = self.create_server_command(method, num_spec_tokens, **kwargs)
    ...
    def wait_for_server_ready_sync(self, timeout: int = 300) -> bool:
        """Synchronous version of wait_for_server_ready."""
        try:
            return asyncio.run(self.wait_for_server_ready(timeout=timeout))
        except Exception as e:
            print(f"Error waiting for server: {e}")
            return False

    async def wait_for_server_ready(self, timeout: int = 300) -> bool:
        """Wait for server to be ready using health check."""
        if not aiohttp:
            print("Warning: aiohttp not available, cannot check server readiness")
            return False
    ...
    def generate_prompts(
        ...
        base_prompt = "Explain the concept of quantum computing in detail."
    ...
    async def run_api_benchmark(
        ...
        """Run benchmark using OpenAI API client."""
        ...
        results = {
            ...
            "memory_gb": {
                "mean": 0.0,  # VLLM server memory tracking not available via API
                "max": 0.0,
                "utilization_pct": 0.0,
            },
    ...
    def run_test_scenario(
        ...
        **kwargs
        ...
        if not self.start_server(method, num_spec_tokens, **kwargs):
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark VLLM speculative decoding performance using server"
    )
    ...
    parser.add_argument(
        "--method",
        choices=["all", "baseline", "eagle", "eagle3", "ngram", "suffix"],
        default="all",
    ...
    print(f"VLLM Server Benchmark Suite")
    print(f"VLLM Version: {VLLMServerBenchmark(args.model_path, args.spec_model_path, args.output_dir).vllm_version}")

    runner = VLLMServerBenchmark(args.model_path, args.spec_model_path, args.output_dir)
