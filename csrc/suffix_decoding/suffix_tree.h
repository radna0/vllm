...
#include <deque>
#include <memory>
// #include <span> // Not available in C++17
#include <cstddef>
#include <utility>
#include <vector>
...
public:
    ...
    Span<T> subspan(size_t offset, size_t count = static_cast<size_t>(-1)) const {
        ...
        if (count == static_cast<size_t>(-1) || offset + count > size_) count = size_ - offset;
...
private:
    ...
    std::pair<Node*, int> _match_context(Span<const int32_t> context);
